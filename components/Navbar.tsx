...
function AIAssistant({ theme }: { theme: Theme }) {
  ...
  const [messages, setMessages] = useState<{ role: "user" | "ai"; text: string }[]>([
    { role: "ai", text: "Cześć! Pomogę Ci znaleźć firmy. Napisz np. 'producenci mebli z Poznania' albo 'firmy IT z Warszawy zatrudniające 10-50 osób'." }
  ])
  ...
  function handleSend() {
    ...
    // Placeholder — w przyszłości tu będzie Claude API
    setTimeout(() => {
      setMessages(prev => [...prev, {
        role: "ai",
        text: "Szukam firm pasujących do Twojego opisu... Ta funkcja będzie dostępna wkrótce w planie Basic i Pro. Wpisz NIP lub nazwę firmy w wyszukiwarce żeby znaleźć konkretną firmę."
      }])
    }, 800)
  }
      ...
      {open && (
        <div style={{
          ...
          <div style={{ padding: "12px 16px", borderBottom: `1px solid ${border}`, display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div>
              <p style={{ fontSize: 13, fontWeight: 600, color: text, margin: 0 }}>Asystent nipgo</p>
              <p style={{ fontSize: 11, color: muted, margin: 0 }}>Znajdę firmy dla Ciebie</p>
            </div>
          </div>
          ...
          {user ? (
            ...
            <div style={{ position: "relative" }}>
              <button
                ...
                style={{
                  ...
                  border: `1px solid ${btnOutlineBorder}`,
                  borderRadius: 10, background: btnOutlineBg,
                  cursor: "pointer",
                }}
              >
                {/* Avatar */}
              ...
              {menuOpen && (
                <div style={{
                  ...
                  {[
                    { icon: <LayoutDashboard size={13} />, label: lang === "pl" ? "Dashboard" : "Dashboard", href: "/dashboard" },
                    { icon: <LogOut size={13} />, label: lang === "pl" ? "Wyloguj się" : "Sign out", href: null },
                  ].map((item, i) => (
                    item.href ? (
            ...
            <>
              <Link href="/login" style={{
                ...
                border: `1px solid ${btnOutlineBorder}`,
                borderRadius: 10, background: btnOutlineBg, color: btnOutlineColor,
                textDecoration: "none",
              }}>
